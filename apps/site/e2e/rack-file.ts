 ...
 */
export function writeRackFile(kind: 'instrument' | 'drum' | 'plugin' | 'plugin-mapped' | 'handmade' = 'instrument'): string {
  ...
  // A drum rack with KICK GAIN on one pad's Utility, made by hand - the case
  // the contract has to ask about rather than quietly empty.
  if (kind === 'handmade') return donor('KD.adg');
